<template>
  <div class="cloud-app-page">
    ...
    <section class="overview-band" aria-label="转存概览">
      ...
      <div class="overview-metrics">
        <div
            ...
            :class="[
              'overview-metric',
              {'overview-metric--desktop-only': ['成功', '失败'].includes(stat.title)},
            ]"
        >
    ...
    <section
        :class="['workspace', {'workspace-history': mainTab === 'history'}]"
    >
...
function notify(text, type = "success") {
  const method = ["success", "info", "warning", "error"].includes(type)
      ? type
      : "success";
...
async function playHistory(itemId) {
  ...
  try {
    const result = await api.get(
        `plugin/${props.pluginId}/history/play/${encodeURIComponent(itemId)}`,
    );
