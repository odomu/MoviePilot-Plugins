<template>
  ...
  >
    <v-card>
      <v-card-title
          class="text-subtitle-1 d-flex align-center px-3 py-2 bg-primary-lighten-5"
      >
...
const scanHint = computed(
    () => providerMeta[props.provider]?.hint || "请扫描二维码完成登录",
);
...
async function loadQrCode() {
  ...
  try {
    ...
    const result = unwrapResponse(
        await props.api.get(`plugin/${pluginId}/qrcode?${query}`),
    );
    if (result.success === false) {
    ...
    const channelName =
        session.value.channel_name ||
        channels.find((item) => item.value === channel.value)?.title ||
        "115客户端";
    statusText.value = props.provider === "115"
        ? `等待${channelName}扫码`
        : "等待扫码";
    schedulePolling();
  } catch (loadError) {
...
watch(
    ...
    ([visible]) => (visible ? loadQrCode() : stopPolling()),
);
watch(
    channel,
